...
import HeaderTeacherLayout from './HeaderTeacherLayout';
...
const listMenuItem: MenuProps['items'] = [
  getItem('Lớp học phụ trách', 'class', <UserOutlined style={{ fontSize: 18 }} />),
];

export default function TeacherLayout({ children }: LayoutProps) {
  const [collapsed, setCollapsed] = useState<boolean>(false);
  const [selectedKeyMenu, setSelectedKeyMenu] = useState<string>('class');
  ...
  const handleClickMenu = (value: MenuInfo) => {
    if (value.key !== selectedKeyMenu) {
      if (value.key === 'class') {
        ...
        router.push({ pathname: `/teacher/${value.key}` });
      }
  ...
  return (
    <Layout hasSider style={{ backgroundColor: '#fff', minHeight: '100vh' }}>
      ...
      >
        <HeaderTeacherLayout collapsed={collapsed} />
